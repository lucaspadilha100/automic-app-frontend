...
export default function MasterSupportPage() {
  ...
  // Only the id is held in state. The list endpoint returns tickets without their
  // messages, and a copy of a list row also goes stale the moment the ticket
  // changes — so the open ticket is always read from its own query.
  ...
  const { data: selected } = useQuery({
    queryKey: ['master','support','ticket', selectedId],
    queryFn: () => masterApi.getSupportTicket(selectedId!),
    enabled: !!selectedId,
  })

  const invalidateTicket = () => {
    qc.invalidateQueries({ queryKey: ['master','support'] })
    qc.invalidateQueries({ queryKey: ['master','support','ticket', selectedId] })
  }

  const replyMut = useMutation({
    mutationFn: () => masterApi.replyTicket(selectedId!, reply),
    onSuccess: () => {
      invalidateTicket()
      setReply('')
  ...
  const statusMut = useMutation({
    ...
    onSuccess: () => { invalidateTicket(); toast.success('Status do ticket atualizado') },
    onError: (e) => toast.error(extractApiError(e)),
  })
            ...
            <div className="space-y-2">
              {tickets?.map((t: Record<string,unknown>) => (
                <div key={t.id as string}
                  ...
                  className={`card p-4 cursor-pointer transition-all ${selectedId === t.id ? 'border-primary-400 ring-1 ring-primary-200' : 'hover:border-slate-300'}`}>
        ...
        {selected && (
          <div className="lg:col-span-2 card flex flex-col">
            ...
            <div className="flex-1 p-5 space-y-3 max-h-80 overflow-y-auto">
              {(selected.messages as {id:string;body:string;author_side:string;created_at:string}[] || []).map(m => (
                <div key={m.id} className={`rounded-lg p-3 text-sm max-w-[80%] ${m.author_side === 'automic' ? 'ml-auto bg-primary-50 text-primary-900' : 'bg-slate-100 text-slate-800'}`}>
                  <p>{m.body}</p>
